...
@Component
public class UnSubscribeEventMsgHandler implements MsgHandler {
	private Logger logger = LogManager.getLogger(this.getClass());

	@Override
	public boolean canHandle(UpMessage message) {
		return "event".equals(message.getMsgType()) && "unsubscribe".equals(message.getEvent());
	}

	@Override
	public String handle(UpMessage message) {
		logger.info("{}取消关注了{}", message.getFromUserName(), message.getToUserName());
